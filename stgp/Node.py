...
class Node:
	...
	def calculate(self, sample):
		...
		if self.left == None:
			...
			if self.value == "max":  #max
				return np.maximum(self.left.calculate(sample), self.right.calculate(sample))
			if self.value == "min":  #min
				return np.minimum(self.left.calculate(sample), self.right.calculate(sample))

	def fromString(self, string, index, operators, terminals):
		'''
		...
		if string[index[0]] == "(":
				index[0] += 1
				self.left = Node()
				self.left.fromString(string, index, operators, terminals)
				self.value = string[index[0]]
				index[0] += 1
				self.right = Node()
				self.right.fromString(string, index, operators, terminals)
				index[0] += 1 # ")"
		else:
				self.value = string[index[0]]
				index[0] += 1
